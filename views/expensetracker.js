...
async function addNewExpense(e){
   ...
   const expense= await axios.post('http://54.88.101.55:4000/expense/addexpense',expenseDetails,{headers:{'Authorization':token}})
    console.log(expense)
...
window.addEventListener('DOMContentLoaded',async()=>{
   ...
   const response = await axios.get(`http://54.88.101.55:4000/expense/data?page=${page}=${ltd}`,{headers:{'Authorization':token}})
...
function deleteExpense(e,expenseid){
    try{
    const token = localStorage.getItem('token')
    axios.delete(`http://54.88.101.55:4000/expense/deleteexpense/${expenseid}`,{headers:{'Authorization':token}})
    .then(()=>{
...
document.getElementById('rzp-button1').onclick = async function (e) {
    try{
    const token = localStorage.getItem('token')
    const response  = await axios.get('http://54.88.101.55:4000/purchase/premiummembership', { headers: {"Authorization" : token} });
    console.log(response);
    var options =
    {
     ...
     "handler": async function (response) {
        const res = await axios.post('http://54.88.101.55:4000/purchase/updatetransactionstatus',{
...
function showLeaderboard(){
    ...
    inputElement.onclick = async() => {
        ...
        const token = localStorage.getItem('token')
        const userLeaderBoardArray = await axios.get('http://54.88.101.55:4000/premium/showLeaderBoard', { headers: {"Authorization" : token} })
...
function download(){
    try{
    const token = localStorage.getItem('token')
    axios.get('http://54.88.101.55:4000/expense/download', { headers: {"Authorization" : token} })
    .then((response) => {
...
const expense = await axios.get(`http://54.88.101.55:4000/expense/data?page=${page}=${ltd}`, { headers: { 'Authorization': token }})
        console.log(expense)
